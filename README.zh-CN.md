...
- **/music**  
  通过内置的 `yt-dlp` + `ffmpeg` 流程，从 YouTube 搜索并下载音频。  
  支持关键词或直接链接；搜索结果以 inline 按钮列出，选中的曲目以带元数据和封面的音频文件发送。  
  每次下载与搜索都会按用户持久化记录，作为推荐引擎的信号。

- **/randommusic**  
  个性化歌曲推荐。  
  从用户的下载历史、搜索关键词以及习得的艺人偏好中构造种子，再用加权随机抽样对候选排序（连续调用结果会变化，不会重复）。  
  推荐以音频形式发送，附 👍 / 👎 反馈按钮：👍 提高该艺人权重，👎 撤回该条消息并降低权重。  
  手动调用返回 **1 首**；`/randommusic on|off` 可开关每日自动推送（北京时间早 8 点推 **3 首**）。
...
- **功能模块**
  - 独立的命令处理逻辑  
  - AI 图像生成模块（火山引擎 / 豆包）  
  - 抓歌与推荐模块（`yt-dlp` + `ffmpeg`，按用户的 SQLite 持久化）  
  - 预留用于后续功能扩展的模块接口  

- **配置管理**
  - 所有配置项与密钥均通过环境变量注入  
  - 代码仓库中不包含任何敏感信息  
  - 按用户的数据（下载历史、偏好、订阅）持久化于挂载的 SQLite 卷中  
...
## 后续计划
- 引入 SoundCloud 作为下载 / 推荐来源（待稳定的提取 handler 就绪后接入）  
